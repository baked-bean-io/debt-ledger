...
1. **Never edit `.techdebt/items.json` by hand.** Every write goes through
   the CLI (`debt add`, `debt status`, `debt triage`). The ledger
...
3. **Estimates use the calibration rubric.** Run `debt rubric` and quote
...
The `debt` command-line tool ships inside this skill — nothing needs to
be installed. Wherever this document says `debt ...`, run:

    node "${CLAUDE_SKILL_DIR}/bin/debt.cjs" ...

`${CLAUDE_SKILL_DIR}` resolves to this skill's own directory. If the machine
also has a global `debt` command on PATH, it is the same tool and may be
...
2. Read the code. Look for: design flaws that force workarounds, missing
   ...
   security smells. `debt scan` harvests TODO/FIXME comments if useful
   as a starting map — but your value is finding debt no comment marks.
3. Run `debt rubric` and `debt report --json` (to check what's
...
6. Run `debt add --file <tmpfile>`. Report the minted ids. Remind the
...
- For scan candidates: `debt scan --json > <your-scratchpad>/candidates.json` (a temp file in your scratchpad, never in the repo),
...
- For re-estimating an existing item: prefer telling the user to run
  `debt triage --revisit <id>` (interactive, shows current values). If
  they want to stay in conversation: show the item from
  `debt report --json`, agree on new values, then — since `add` creates
...
2. Mid-work: `debt suggest --files <comma-list> --json`.
   ...
   then `debt suggest --max-effort <n> --json`.
...
- `debt status <id> fixed` (or `planned` / `wontfix` / back to `open`),
  after confirming the id via `debt report --json` if ambiguous.
...
- `debt scan` warns on stderr about items whose locations no longer
...
- Items whose rank leans on `blocksWork` show a "claims to block" warning
  in `debt report` — if a claimed blocker shipped, offer to prune it
  via `--revisit`.
- If any debt command reports the ledger is unreadable or has duplicate
  ids (typically right after a git merge), run `debt doctor` to see
  what's wrong and `debt doctor --fix` to repair it. Doctor without
